...
Saajan avattu näkymä: lämmin fabric jatkuu pehmennettynä (`::before` blur ~10px + cream/wine-vignette) — ei enää tasainen cream-drop. Osioiden paneelit (`osio` / `tint-0` cream, `tint-1` rose, `tint-2` wine-wash) saavat näkyvän reunan ja sisäpaddingin. Pukeutumisvihje vastaanottajalle: **Pukeudu näin** cream-korttina (`#FFFDF9`, `rgba(74,31,44,.16)`, pehmeä varjo). Debug-footer pysyy poissa. Peel-pino, Tapaaminen ja modal ennallaan.
