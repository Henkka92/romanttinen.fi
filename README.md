...
Saajan avattu näkymä: lämmin fabric jatkuu pehmennettynä (`::before` blur + cream/wine-vignette, ilmapiiri korttien takana — ei tapettia, ei tasainen cream-drop). Osioiden paneelit (`osio` / `tint-0` cream, `tint-1` rose, `tint-2` wine-wash) saavat näkyvän reunan ja sisäpaddingin. Pukeutumisvihje vastaanottajalle: **Pukeudu näin** cream-korttina (`#FFFDF9`, `rgba(74,31,44,.16)`, pehmeä varjo). Debug-/versio-footer pysyy poissa; chrome on `Kutsu avattu`. WhatsApp/iMessage: OG-tagit (`Sinut on kutsuttu treffeille.`, pehmeä kuvaus, teaser-kuva — ei spoilereita). Peel-pino, Tapaaminen ja modal ennallaan.
